...
자연어 → Graph 변환 후 Research Team 아키텍처로 실행. **3단계 이상 + 분기/병렬/반복이 있을 때 사용.**

**Schema v2**: `{id, goal, state:{workspace, raw_vault}, nodes:[{id, do, with, reads, writes, artifacts, autonomy}], flow, limits}`
...
**실행**: Graph 생성 → 사용자 확인 → 노드 순회 → workspace에 누적 → `.omc/state/graph/{id}/`에 저장
...
`output styles` → plugins, `legacy SDK` → `@anthropic-ai/claude-agent-sdk`, `includeCoAuthoredBy` → `attribution`, SSE → HTTP
