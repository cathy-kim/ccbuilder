...
- **Claude Code v2.1.92 sync**
  ...
  - Remote Control 세션명 호스트명 기본 접두사 (`myhost-graceful-unicorn`), `--remote-control-session-name-prefix`로 오버라이드
  - Pro 사용자: 프롬프트 캐시 만료 후 세션 복귀 시 미캐시 토큰 수 푸터 힌트 표시
- **Claude Code v2.1.91 sync**
  - MCP 도구 결과 크기 오버라이드 — `_meta["anthropic/maxResultSizeChars"]` 어노테이션으로 최대 500K까지 허용 (DB 스키마 등 대형 결과)
  - `disableSkillShellExecution` 설정 — Skills/커스텀 슬래시 명령/플러그인 명령의 인라인 셸 실행 비활성화
  - Deep link `claude-cli://open?q=` 멀티라인 프롬프트 지원 (`%0A` 인코딩 줄바꿈)
  - 플러그인 `bin/` 디렉토리 실행 파일 — Bash 도구에서 bare 명령으로 호출 가능

### Changed
- SKILL.md: 핵심 변경 사항 섹션 v2.1.90 → v2.1.92 업데이트
  - MCP 테이블: `_meta["anthropic/maxResultSizeChars"]` 도구 결과 크기 오버라이드 추가
  - CLI: `/release-notes` 인터랙티브 피커, Remote Control 호스트명 접두사, Bedrock 설정 마법사 추가
  - Plugin: `bin/` 실행 파일 지원 추가
  - 신규 도구·env: `forceRemoteSettingsRefresh`, `disableSkillShellExecution`, deep link 멀티라인 추가
  - Breaking Changes: `/tag` 제거, `/vim` 제거 (→ `/config` Editor mode) 추가
  - Deprecated 테이블: `/tag`, `/vim` 추가
- `references/version-sync.md`: v2.1.92, v2.1.91 변경사항 추적 엔트리 추가

### Fixed (Claude Code v2.1.92 주요 수정)
- tmux 창 종료·번호 재매기 후 서브에이전트 스폰 영구 실패 ("Could not determine pane count") 수정
- 소형 패스트 모델 `ok:false` 반환 시 프롬프트 타입 Stop Hook 잘못 실패하던 버그 수정; `preventContinuation:true` 의미 복원
- 스트리밍에서 배열/객체 필드를 JSON 인코딩 문자열로 전송 시 도구 입력 검증 실패 수정
- Extended thinking 빈 텍스트 블록 포함 시 API 400 오류 수정
- Write 도구 diff 계산 속도 60% 향상 (탭/`&`/`$` 포함 대용량 파일)
...
- Edit 도구 `old_string` 앵커 최적화 — 출력 토큰 절감
