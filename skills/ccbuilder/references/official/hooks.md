...
| # | Event | 용도 | 차단 가능 |
|---|-------|------|-----------|
| 1 | `SessionStart` | 세션 시작/재개 — `reloadSkills: true` 반환으로 스킬 재스캔; `hookSpecificOutput.sessionTitle`로 시작·재개 시 세션 제목 설정 (v2.1.152); resume 시 세션 신선도(staleness)·예상 재캐싱 비용 필드 포함 (v2.1.251) | - |
