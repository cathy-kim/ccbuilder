...
| 9 | `SubagentStop` | 서브에이전트 완료 — `background_tasks`·`session_crons` 입력 필드 포함 (v2.1.145); `hookSpecificOutput.additionalContext` 반환으로 피드백 제공 후 턴 유지 가능 (v2.1.163) | block |
| 10 | `Stop` | Claude 응답 완료 — `background_tasks`·`session_crons` 입력 필드 포함 (v2.1.145); `hookSpecificOutput.additionalContext` 반환으로 피드백 제공 후 턴 유지 가능 (v2.1.163) | block |
