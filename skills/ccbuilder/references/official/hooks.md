...
**Last Synced**: 2026-08-31 (v2.1.251)
...
| # | Event | 용도 | 차단 가능 |
|---|-------|------|-----------|
| 1 | `SessionStart` | 세션 시작/재개 — `reloadSkills: true` 반환으로 스킬 재스캔; `hookSpecificOutput.sessionTitle`로 시작·재개 시 세션 제목 설정 (v2.1.152); resume 시 세션 staleness·예상 re-cache 비용 필드 전달 (v2.1.251) | - |
...
| 9 | `SubagentStop` | 서브에이전트 완료 — `background_tasks`·`session_crons` 입력 필드 포함 (v2.1.145); `hookSpecificOutput.additionalContext` 반환으로 Claude에 피드백 전달하며 턴 계속 (v2.1.163) | block |
| 10 | `Stop` | Claude 응답 완료 — `background_tasks`·`session_crons` 입력 필드 포함 (v2.1.145); `hookSpecificOutput.additionalContext` 반환 지원 (v2.1.163) | block |
...
| 28 | `PreModelSwitch` | 모델 전환 직전 (v2.1.251) | block/confirm/annotate |
| 29 | `PostModelSwitch` | 모델 전환 직후 (v2.1.251) | annotate |
