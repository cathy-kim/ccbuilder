...
| 28 | `PreModelSwitch` | 모델 전환 직전 발동 — 차단·확인·주석(annotate) 가능 (v2.1.251) | block/confirm |
| 29 | `PostModelSwitch` | 모델 전환 완료 후 발동 (v2.1.251) | - |

**`SessionStart` resume 확장 (v2.1.251)**: 세션 재개 시 hook 입력에 세션 staleness(경과 시간)·예상 재캐싱 비용 필드 추가
