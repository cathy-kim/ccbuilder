...
| `EnterWorktree` | - | 격리된 git worktree 세션 진입; `path` 파라미터로 기존 worktree 재사용 가능 (v2.1.105); 신규 브랜치를 `origin/<default>` 대신 local HEAD 기반 생성 수정 — unpushed commit 유실 방지 (v2.1.128) | 낮음 |
