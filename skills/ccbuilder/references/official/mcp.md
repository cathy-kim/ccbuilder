...
| **OAuth 2.0** | `/mcp` 명령으로 인증 (자동/수동 등록); CIMD/SEP-991 지원 — Dynamic Client Registration 없는 서버도 지원 (v2.1.81); 사인인 필요한 서버는 실제 인증 전까지 OAuth 클라이언트 미등록 (v2.1.265); 로컬 콜백 포트 바인딩 실패로 "No available ports" 오류 나던 버그 수정 (v2.1.268) |
| **동기화 플러그인 MCP 서버** | 원격 세션 재개 시 연결되지 않던 버그 수정 (v2.1.269) |
