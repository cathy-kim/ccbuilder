...
**Last Synced**: 2026-09-13 (v2.1.270)
...
- **Workflow 도구 동시 에이전트 상한 (v2.1.269)**: `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`(1–256)로 Workflow 도구 실행당 동시 에이전트 상한 상향 가능 — 추론 병목 fan-out용
- **작업 추적 도구 모델 제한 (v2.1.268)**: `TaskCreate`/`TaskUpdate`/`TaskList`/`TaskGet`·`TodoWrite`는 Claude 3.x·Opus 4.0–4.7·Sonnet 4.0–4.6·Haiku 4.5에서만 기본 제공 — 다른 모델은 `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` 필요
