...
**effort 필드 (v2.1.133)**: 모든 Hook 이벤트 JSON 입력에 `effort.level` 필드가 추가됩니다. Bash 도구 서브프로세스에서는 `$CLAUDE_EFFORT` 환경변수로도 읽을 수 있습니다. 가능한 값: `low`, `medium`, `high`, `xhigh` (Opus 4.7 전용).
