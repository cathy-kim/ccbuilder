...
|-------|------------|--------------|---------------|
| **SessionStart** | 세션 시작/재개 — `reloadSkills: true` 반환으로 스킬 재스캔 (v2.1.152); `hookSpecificOutput.sessionTitle`로 시작·재개 시 세션 제목 설정 (v2.1.152); resume 시 세션 staleness·예상 재캐시 비용 필드 포함 (v2.1.251) | No | `session_id`, `agent_type` |
...
| **PostModelSwitch** | 모델 전환 완료 후 — 주석(annotate) 가능 (v2.1.251) | No | `from_model`, `to_model` |
...
**PreModelSwitch/PostModelSwitch (v2.1.251)**: `/model` 전환·auto mode 모델 변경 등 모델 전환 이벤트를 가로채 차단·확인·주석 가능; `SessionStart` resume 이벤트에 세션 staleness·예상 재캐시 비용 필드 추가
