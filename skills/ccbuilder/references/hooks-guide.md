...
| **PreModelSwitch** | 모델 전환 직전 — 차단·확인·주석 가능 (v2.1.251) | Yes (allow/deny/confirm) | `from_model`, `to_model` |
| **PostModelSwitch** | 모델 전환 직후 (v2.1.251) | No | `from_model`, `to_model` |

**SessionStart resume 필드 확장 (v2.1.251)**: resume 시 세션 staleness(경과 시간)·예상 재캐시 비용 필드가 입력에 포함됩니다.
