...
- **SessionEnd 타임아웃** (v2.1.74): 기존 1.5초 강제 종료 → `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` 환경변수로 가변 설정 가능; 개별 hook에 `timeout`을 지정하지 않으면 이 환경변수가 적용되지 않고 1.5초에 강제 취소되던 버그 수정 (v2.1.268)
