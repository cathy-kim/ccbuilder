...
| **PostModelSwitch** | 모델 전환 완료 후 (v2.1.251) | No | `from_model`, `to_model` |

**SessionStart resume 확장 (v2.1.251)**: 세션 재개(`resume`) 시 hook 입력에 세션 staleness(경과 시간)·예상 재캐싱 비용 필드 추가 — 오래된 세션 재개 시 컨텍스트 판단에 활용
