...
| **MessageDisplay** | 어시스턴트 메시지 텍스트 표시 시 — 텍스트 변환·숨김 처리 가능 (v2.1.152) | Yes (transform/hide) | `message` |
...
**SessionStart 신규 기능 (v2.1.152)**: `reloadSkills: true` 반환 — 훅이 설치한 스킬을 동일 세션에서 즉시 사용 가능 (재시작 불필요); `hookSpecificOutput.sessionTitle` — 세션 시작·재개 시 제목 설정 지원 (기존 UserPromptSubmit Hook에서 확장)

**MessageDisplay Hook (v2.1.152)**: 어시스턴트 메시지 텍스트가 표시되기 전 변환하거나 숨길 수 있는 새 Hook 이벤트 (#26)
