...
### v2.1.266 (2026-09-09 동기화)

**새로운 기능:**
- (v2.1.265) `--plugin-dir`이 플러그인 폴더 하나를 가리킬 때 매니페스트 있는 각 하위 폴더 자동 로드 — 실행 중 추가/제거되는 하위 폴더도 감지
- (v2.1.265) 도구 결과 디스크 저장 1GB 상한 신규 — 대화 내 미리보기에 잘림 여부 표시
- (v2.1.265) Claude Desktop·Cowork가 Claude apps gateway로 보내는 텔레메트리에 `user.email`·`user.groups` 추가 (터미널 세션과 동일)
- (v2.1.265) MCP `http` 서버가 레거시 HTTP+SSE transport만 지원할 때 자동 SSE 폴백 — MCP 스펙 준수
- (v2.1.265) `--worktree` 대형 레포 시작 속도 개선 — 신규 worktree 병렬 체크아웃 (git 2.32+)
- (v2.1.265) `/workflows` 에이전트 상세 뷰 개선 — 도구 호출 상태(running/failed/done), 서브에이전트 태스크 목록, Enter로 입력·결과 펼치기
- (v2.1.265) 슬래시 명령 미드프롬프트 입력 시 매치 리스트 표시(Tab으로 풀스크린 밖 오픈), 플러그인 스킬을 bare 이름으로 탐색 가능
- (v2.1.265) 사인인 필요한 원격 MCP 서버 — 실제 인증 전까지 OAuth 클라이언트 미등록
- (v2.1.265) 이미지 크기 초과 시 오류 메시지 개선 — 원인·해결 방법 명시(단순 한도 인용 대신)
- (v2.1.265) Artifact 도구가 타인이 작성한 아티팩트를 읽을 때 페이지를 untrusted content로 처리 — 임베디드 지시문 그대로 전달하지 않고 플래그
- (v2.1.265) `.claude` 폴더 권한 옵션 설명 개선 — 프로젝트/`~/.claude` 폴더 세션 중 편집 허용임을 명시
- (v2.1.265) Claude apps gateway 세션 OTel을 게이트웨이 relay 대신 managed settings에 지정된 컬렉터로 직접 export (컬렉터 미지정 세션은 기존대로 relay 경유)
- (v2.1.265) [VSCode] 유휴 세션 자동 아카이빙(기본 14일, 신규 설정)
- (v2.1.264) 버그 수정 및 안정성 개선 (세부 사항 미공개)
- (v2.1.266) `CLAUDE_CODE_USE_GATEWAY` 단독 설정 시 Cloud gateway 강제 로그인되던 2.1.265 회귀 수정 — API 키·`apiKeyHelper`·커스텀 인증 헤더 병행 설정 시 정상 동작 복원

**Breaking Changes:**
- 없음 (이번 릴리스는 버그 수정·안정성 개선 위주)

**주요 버그 수정:**
- LLM-gateway·프록시 설정에서 `CLAUDE_CODE_USE_GATEWAY` 단독 설정 시 모든 요청이 "Not signed in to the Cloud gateway" 오류로 실패하던 2.1.265 회귀 수정 (v2.1.266)
- 포그라운드 파견 서브에이전트 재개 시 도구 목록·시스템 프롬프트 프리픽스가 바뀌어 해당 에이전트 prompt-cache 재사용이 깨지던 버그 수정 (v2.1.265)
- 팀메이트·재개 서브에이전트가 이후 턴에서 SubagentStart 훅 컨텍스트·프리로드 스킬을 프롬프트 프리픽스 밖으로 이동시켜 prompt-cache 재사용이 깨지던 버그 수정 (v2.1.265)
- 이전 프로세스가 도구 실행 중 종료된 뒤 재개 시 마지막 프롬프트가 재작성되지 않고, 중단된 도구 호출이 interrupted로 표시되며 유지되도록 수정 (v2.1.265)
- `/model opusplan[1m]`이 "Model not found"로 거부되던 버그 수정 (v2.1.265)
- 권한 프롬프트·메시지의 신택스 하이라이트 코드에서 Ruby `?`·Erlang `$`·Perl `$` 시길 뒤 문자가 가끔 생략되던 버그 수정 (v2.1.265)
- 슬래시 명령·@-파일 제안 목록 열고 닫을 때 풀스크린 트랜스크립트가 한 줄씩 튀던 버그 수정 (v2.1.265)
- 플러그인 경로에 백슬래시 포함 시 macOS/Linux 심볼릭 링크 격리 검사 우회되던 보안 버그 수정 (v2.1.265)
- 두 글자(`..`)로 시작하는 플러그인 디렉토리 이름이 플러그인 루트 밖으로 잘못 거부되던 버그 수정 (v2.1.265)
- VS Code·SDK 세션이 토큰 갱신 중 세션 종료 시 간헐적 재로그인 요구되던 버그 수정 (v2.1.265)
- Remote Control 세션이 응답 마지막 메시지 도착 전 턴 종료 신호를 보내 Claude 앱에서 응답이 조기 완료로 표시되던 버그 수정 (v2.1.265)
- 백그라운드(`--bg`) 세션이 유휴 타임아웃 직전 메시지 도착 시 턴 중간에 retire되던 버그 수정 (v2.1.265)
- Claude Code 자체 git status/diff probe가 작업 트리 내 중첩 레포의 clean filter를 실행하던 버그 수정 (v2.1.265)
- `/add-dir <subdirectory>` — managed settings가 스킬만 플러그인으로 잠갔을 때 서브디렉토리 에이전트 로드가 거부되던 버그, 에이전트만 잠겼을 때 잘못 약속되던 버그 수정 (v2.1.265)
- tmux 등에서 두 번째 키 입력이 1초 이상 늦으면 조용히 취소되던 2키 단축키 — 이제 3초 대기 후 타임아웃 안내 표시 (v2.1.265)
- forked skills(`context: fork`)가 kickoff 프롬프트를, `--forward-subagent-text` 사용 시 텍스트 턴을 stream-json progress 이벤트로 스트리밍하지 않던 버그 수정 (v2.1.265)
- claude-api 스킬 오류 코드 레퍼런스 수정 — 모델 접근 실패는 404, 미지원 beta 헤더는 400 (403 아님) (v2.1.265)
- MCP `http` transport로 설정했지만 레거시 HTTP+SSE만 지원하는 서버가 아예 연결되지 않던 버그 수정 — SSE 폴백 (v2.1.265)
- claude.ai 커넥터가 HTTP 401을 반환할 때 실제로는 연결됐음에도 클라우드 세션에서 인증 필요로 잘못 표시되던 버그 수정 (v2.1.265)
- 원격 세션이 커넥터 승인·로그인 링크 대기 중 샌드박스 컨테이너를 계속 살려두던 버그 수정 (v2.1.265)
- 재개 세션의 "백그라운드 태스크 미완료" 알림이 짧은 상태 라인 대신 긴 모델용 복구 지침을 표시하던 버그 수정 (v2.1.265)
- Windows: AppContainer·제한 토큰 샌드박스에서 Read/Write/Edit이 "symlink resolution changed after permission was checked" 오류로 전부 거부되던 버그 수정 (v2.1.265)
