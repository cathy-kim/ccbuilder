...
### v2.1.148 (2026-05-22 동기화)

**새로운 기능:**
- (v2.1.147) 핀드 백그라운드 세션 (`Ctrl+T` in `claude agents`) — 유휴 시 유지·Claude Code 업데이트 적용 자동 재시작·메모리 압박 시 비핀드 세션 우선 제거
- (v2.1.147) `/code-review --comment` — 수정사항을 인라인 GitHub PR 코멘트로 게시
- (v2.1.147) 자동 업데이터 개선 — 일시적 네트워크 오류 재시도·에러 카테고리·OS 에러 코드 상세 보고·업데이트 실패 시 현재 버전 표시
- (v2.1.147) 프롬프트 히스토리 연속 중복 항목 미기록 — 동일 프롬프트 재제출 시 히스토리 중복 방지
...
- Hook `if` 조건 `PowerShell(git push*)` 등 미매칭 수정 (v2.1.147)
