...
- 플러그인 `bin/` 실행파일 동봉 지원 — Bash 도구에서 bare 명령으로 직접 호출 가능
- `/feedback` 명령이 비활성화 이유를 설명하도록 개선 (슬래시 메뉴에서 사라지지 않음)
...
**주요 버그 수정:**
- `--resume` 비동기 트랜스크립트 쓰기 실패로 대화 이력 유실되던 체인 단절 수정
