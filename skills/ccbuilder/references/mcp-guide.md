...
> **v2.1.243 수정**: 비대화형(`-p`) 및 SDK 세션에서 원격 MCP 서버가 연결이 끊긴 뒤 복구되지 않던 버그 수정 — 이제 자동으로 재연결을 시도하거나 실패로 보고합니다.
...
> **v2.1.243**: `/mcp`와 `/plugins`에서 조직이 인증을 관리하는 claude.ai 커넥터에 `managed` 표시가 추가되어, 어떤 연결이 조직 관리 대상인지 한눈에 구분할 수 있습니다.
