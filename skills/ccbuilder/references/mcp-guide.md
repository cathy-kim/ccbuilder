...
> **v2.1.231 수정**: 사전 등록된 OAuth 클라이언트를 사용하는 서버(예: Slack)에서 redirect URI mismatch로 sign-in이 실패하던 버그 수정.

> **v2.1.243 수정**: 클라이언트 ID 메타데이터 문서(CIMD)를 지원하는 서버에서, 데스크탑 앱에서 시작한 MCP 서버 sign-in이 "Invalid redirect URI"로 실패하던 버그 수정.

> **v2.1.243 개선**: `-p`/SDK 세션에서 연결이 끊긴 원격 MCP 서버가 영구적으로 미복구 상태로 남던 문제 수정 — 이제 자동으로 재연결되거나 명확히 실패로 보고됩니다.
