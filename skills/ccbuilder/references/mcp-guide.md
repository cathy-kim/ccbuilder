...
`workspace`는 이제 예약된 MCP 서버 이름입니다. 이 이름을 사용하는 기존 서버는 경고와 함께 스킵됩니다.

> **Breaking Change**: `.mcp.json` 또는 설정에서 `workspace`라는 이름의 MCP 서버를 사용 중이라면 다른 이름으로 변경하세요.

### 재연결 시 도구 목록 홍수 방지

MCP 서버가 재연결될 때 더 이상 전체 도구 이름 목록을 대화에 홍수처럼 표시하지 않습니다. 재공지된 도구는 서버 접두사별로 요약되어 표시됩니다.

### `/mcp` 도구 수 표시

`/mcp` 명령이 연결된 서버별 도구 수를 표시하고, 0개 도구로 연결된 서버를 별도로 표지합니다.

### stdio 서버 아규먼트 오염 수정

`CLAUDE_CODE_SHELL_PREFIX`가 설정된 환경에서 MCP stdio 서버로 전달되는 아규먼트에 공백이나 셸 메타문자가 포함될 때 발생하던 아규먼트 오염 버그가 수정되었습니다.
