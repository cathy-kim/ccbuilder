...
## `/mcp` 중복 커넥터 감지 (v2.1.122 신규)

`/mcp` 명령에서 동일한 URL을 가진 로컬 서버에 의해 숨겨진 claude.ai 커넥터를 표시하고, 중복 제거 힌트를 제공합니다.

> **활용**: claude.ai 커넥터가 `/mcp`에 보이지 않을 경우, 동일 URL로 수동 등록된 로컬 서버가 있는지 확인하세요. 중복 서버를 제거하면 커넥터가 다시 표시됩니다.
...
> **v2.1.122 수정**: 비차단(nonblocking) 모드에서 세션 시작 후 연결된 MCP 도구가 ToolSearch에 누락되는 버그가 수정되었습니다.
