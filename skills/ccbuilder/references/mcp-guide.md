...
### `/model` 피커 게이트웨이 모델 목록

`ANTHROPIC_BASE_URL`이 Anthropic 호환 게이트웨이를 가리킬 때, `/model` 피커가 해당 게이트웨이의 `/v1/models` 엔드포인트에서 사용 가능한 모델 목록을 조회합니다.

### `/mcp` 커넥터 중복 표시 개선 (v2.1.122)

동일 URL의 수동 추가 서버에 의해 가려졌던 claude.ai 커넥터가 이제 `/mcp` 목록에 표시되며, 중복 제거 힌트를 함께 제공합니다.

### `allowManagedDomainsOnly` / `allowManagedReadPathsOnly` 보안 수정

상위 우선순위 관리설정 소스에 `sandbox` 블록이 없더라도 이 설정이 올바르게 적용됩니다. 이전에는 해당 케이스에서 설정이 무시되는 보안 버그가 있었습니다.
