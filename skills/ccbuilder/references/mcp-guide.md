...
>
> **v2.1.282**: `anthropic-skills`·`claude-ai` 이름의 MCP 서버는 skills·prompts 목록을 더 이상 제공하지 않음(도구 자체는 계속 동작) — claude.ai 동기화 스킬과의 이름 충돌 방지. 목록을 다시 노출하려면 MCP 설정에서 서버 이름을 변경하세요.
...
**v2.1.281**: URL-mode elicitation(2026-07-28 프로토콜) 추가 — 서버가 브라우저 기반 플로우를 열도록 Claude Code에 요청 가능. 서버가 완료를 확인할 수단이 없을 때는 대기 다이얼로그가 화면에 남지 않고 자동으로 정리됨.
