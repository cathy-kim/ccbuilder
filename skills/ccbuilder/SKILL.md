...
| **claude.ai MCP connectors** | claude.ai MCP 커넥터 사용 (v2.1.46); `ENABLE_CLAUDEAI_MCP_SERVERS=false` 비활성화 (v2.1.63); 로컬 설정과 중복 시 로컬 우선 (v2.1.84); `allowAllClaudeAiMcps` — managed-mcp.json과 함께 claude.ai cloud MCP 커넥터 동시 로드 (v2.1.149); `/mcp`·`/plugins`에 조직 관리 인증 커넥터 `managed` 표시 (v2.1.243) |
