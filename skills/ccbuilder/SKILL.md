...
| **MCP OAuth** | 콜백 포트 충돌·재인증 수정 (v2.1.74); CIMD/SEP-991 (Client ID Metadata Document) 지원 (v2.1.81); RFC 9728 Protected Resource Metadata discovery — 인증 서버 자동 탐색 (v2.1.85); pre-registered OAuth client(Slack 등) redirect URI mismatch 수정 (v2.1.231) |
| **CLAUDE_CODE_MCP_SERVER_NAME/URL** | `headersHelper` 스크립트에서 서버 이름·URL 접근 — 하나의 헬퍼로 다중 서버 처리 (v2.1.85); 플러그인 마켓플레이스 `headersHelper` — 카탈로그·동일 출처 아카이브 fetch용 단기 토큰 헤더 발급, 설치/업데이트 시에만 실행되며 `claude plugin install/update`가 `[y/N]` 확인 요구 (`-y`로 스킵, v2.1.238); project `.mcp.json` MCP `headersHelper`·inline MCP 서버는 폴더 신뢰 다이얼로그 승인 필요, 상속 자격증명 env var 없이 실행 (v2.1.238) |
...
- **Agent 필드**: `isolation: worktree` (격리 실행), `background: true` (백그라운드), `model` (per-invocation 오버라이드 복원, v2.1.72); 전체 모델 ID (`claude-opus-4-5` 등) agent frontmatter에서 수용 (v2.1.74); `initialPrompt` — 에이전트 첫 턴 자동 제출 (v2.1.83); **서브에이전트 포킹(`subagent_type: "fork"`) 기본 활성화** — 전체 대화·프롬프트 캐시 상속, 인터랙티브 세션 non-teammate 에이전트 파견 기본 백그라운드 실행 (v2.1.232)
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.238+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
