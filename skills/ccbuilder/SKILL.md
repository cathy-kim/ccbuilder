...
| **MCP OAuth** | 콜백 포트 충돌·재인증 수정 (v2.1.74); CIMD/SEP-991 (Client ID Metadata Document) 지원 (v2.1.81); RFC 9728 Protected Resource Metadata discovery — 인증 서버 자동 탐색 (v2.1.85); 사인인 필요한 원격 서버에 실제 인증 전까지 OAuth 클라이언트 미등록 (v2.1.265); 로컬 콜백 포트 범위 바인딩 실패 시 "No available ports for OAuth redirect" 오류로 사인인 실패하던 버그 수정 (v2.1.268) |
...
| `TaskGet` | 개별 작업 상세 조회 (Claude 3.x·Opus 4.0–4.7·Sonnet 4.0–4.6·Haiku 4.5에서만 기본 제공, 그 외 모델은 `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` 필요, v2.1.268) |
