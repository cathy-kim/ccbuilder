...
| **--channels** | MCP 서버가 세션에 메시지 푸시 / 채널 서버가 도구 승인 프롬프트 폰으로 릴레이 (v2.1.80-81); console(API key) 인증 지원 (v2.1.128), managed settings 조직은 `channelsEnabled: true` 필요 |
...
| **v2.1.128 MCP 개선** | `workspace` 서버명 예약 → 동명 서버 스킵·경고; 재연결 시 도구 홍수 → 서버별 요약; `/mcp` 서버별 도구 수·0도구 서버 표지; stdio `CLAUDE_CODE_SHELL_PREFIX` 아규먼트 오염 수정 |
...
---
**Status**: 2026-05-03 | **Claude Code**: v2.1.128+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
