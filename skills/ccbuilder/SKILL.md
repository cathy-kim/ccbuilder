...
| **MCP OAuth** | 콜백 포트 충돌·재인증 수정 (v2.1.74); CIMD/SEP-991 (Client ID Metadata Document) 지원 (v2.1.81); RFC 9728 Protected Resource Metadata discovery — 인증 서버 자동 탐색 (v2.1.85); 사인인 필요한 원격 서버에 실제 인증 전까지 OAuth 클라이언트 미등록 (v2.1.265); 로컬 콜백 포트 범위 바인딩 불가 시 "No available ports for OAuth redirect" 오류로 로그인 실패하던 버그 수정 (v2.1.268) |
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.268+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
