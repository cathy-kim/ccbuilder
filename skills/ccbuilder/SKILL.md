...
| **Hook → MCP 직접 호출** | `type: "mcp_tool"` 훅으로 MCP 도구 직접 실행 (v2.1.118); 서브에이전트·SDK 재구성 시 서버 병렬 연결 (v2.1.119); 세션 추가 작업 디렉토리 `roots/list` 전달 (v2.1.203); per-server `request_timeout_ms` 적용 수정 (v2.1.206); "Claude Browser" 서버 이름 예약 (v2.1.205); 2분+ 실행 도구 호출 자동 백그라운드 전환 — `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS` (v2.1.212); 유휴 후 플러그인 MCP 재연결 안 되던 버그 (v2.1.211); OAuth 재인증 시 기존 자격증명 조기 폐기 버그 (v2.1.216); truncated 도구 결과 메모리 누수 수정 (v2.1.217) |
...
| **메모리 타임스탬프** | 메모리 파일 최종 수정 시각 자동 기록 — 신선도 판단 지원 (v2.1.75); ISO `modified` 필드로 정교화 (v2.1.205); 프론트매터 값이 인라인 `#`에서 잘리던 버그 수정 (v2.1.214); MEMORY.md 200줄 초과 시 침묵 절단 대신 명시적 오류 (v2.1.210); 과다 경고가 프론트매터·HTML 주석 제외 로드 콘텐츠만 측정하도록 개선 (v2.1.211) |
...
| `TaskOutput` 도구 / Task tool `mode` 파라미터 | 각각 `Read`로 출력 경로 읽기 (v2.1.83) / 무시됨, 부모 세션 권한 모드 상속 (v2.1.212) |
...
**주요 이벤트**: SessionStart, PreToolUse, PostToolUse, Stop, StopFailure, SubagentStop, TeammateIdle, InstructionsLoaded, PostCompact, Elicitation, ElicitationResult, CwdChanged, FileChanged, TaskCreated, PermissionDenied, MessageDisplay (26개)
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.217+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
