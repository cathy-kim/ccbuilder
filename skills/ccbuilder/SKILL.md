...
| **claude.ai MCP connectors** | claude.ai MCP 커넥터 사용 (v2.1.46); `ENABLE_CLAUDEAI_MCP_SERVERS=false` 비활성화 (v2.1.63); 로컬 설정과 중복 시 로컬 우선 (v2.1.84); `allowAllClaudeAiMcps` — managed-mcp.json과 함께 claude.ai cloud MCP 커넥터 동시 로드 (v2.1.149); `managed` 마커 — 조직이 인증을 관리하는 claude.ai 커넥터를 `/mcp`·`/plugins`에 표시 (v2.1.243) |
...
| **Hook → MCP 직접 호출** | `type: "mcp_tool"` 훅으로 MCP 도구 직접 실행 (v2.1.118); 서브에이전트·SDK 재구성 시 서버 병렬 연결 (v2.1.119); MCP 도구 호출 2분 초과 시 자동 백그라운드 전환 — `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`로 임계값·비활성화 설정 (v2.1.212); SDK MCP 서버 핸드셰이크 확인 유실 시 70초 타임아웃 후 해당 서버만 실패 처리 (v2.1.251); `/mcp` reconnect·enable이 뒤늦게 로드된 managed allow/deny 목록·`strictPluginOnlyCustomization` 차단을 반영 (v2.1.257); `claude mcp remove`가 원격 서버 저장된 OAuth 자격증명까지 정리, WebSocket 연결 실패 로그 실제 오류 메시지 표시, OAuth 로그 자격증명 redact (v2.1.257) |
...
| **메모리 타임스탬프** | 메모리 파일 최종 수정 시각 자동 기록 — 신선도 판단 지원 (v2.1.75); `promptCacheTtl`/`subagentPromptCacheTtl` 설정 — 메인 대화 1시간, 서브에이전트 5분 캐시 TTL 분리 지정 (v2.1.243); Agent frontmatter `experimental.cacheTtl`(`5m`\|`1h`) — 서브에이전트 TTL 미설정 시 사용 (v2.1.248) |
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.258+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
