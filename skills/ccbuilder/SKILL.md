...
| **oauth.authServerMetadataUrl** | MCP OAuth 커스텀 메타데이터 URL 지정 (v2.1.69); 재시작 후 토큰 갱신 시에도 URL 적용 수정 (v2.1.97, ADFS 지원) |
...
- **CLI**: `claude agents`, `claude auth login [--console]`, `claude auth status/logout`, `claude remote-control`, `--worktree (-w)`, Ctrl+F (에이전트 종료), `--bare` (-p 경량화: 훅/LSP/플러그인 비활성화·Auto-memory 끔, API key 필수, v2.1.81), `rate_limits` statusline 필드 (5시간/7일 rate limit 표시, v2.1.80); auto mode 거부 명령 알림 + `/permissions` Recent 탭에서 `r`로 재시도 (v2.1.89); NO_FLICKER `Ctrl+O` Focus view — 프롬프트·툴 요약(diffstats)·최종 응답 토글 (v2.1.97); `refreshInterval` statusline 재실행 주기 설정 (v2.1.97); `workspace.git_worktree` statusline JSON 입력 필드 (v2.1.97); `/agents` `● N running` 실시간 서브에이전트 인디케이터 (v2.1.97)
...
---
**Status**: 2026-03-27 | **Claude Code**: v2.1.97+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
