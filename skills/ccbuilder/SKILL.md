...
| **MCP OAuth** | 콜백 포트 충돌·재인증 수정 (v2.1.74); CIMD/SEP-991 (Client ID Metadata Document) 지원 (v2.1.81); RFC 9728 Protected Resource Metadata discovery — 인증 서버 자동 탐색 (v2.1.85); Slack 등 사전 등록 OAuth 클라이언트 redirect URI mismatch 수정 (v2.1.231); 데스크탑 앱발 CIMD 서버 "Invalid redirect URI" 실패 수정 (v2.1.243); marketplace `headersHelper` — url/catalog 소스 설치·업데이트 시 단명 토큰 등 HTTP 헤더 발급 커맨드 실행 (v2.1.238); `-p`/SDK 세션 원격 MCP 연결 끊김 후 자동 재연결·실패 보고 (v2.1.243) |
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.245+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
