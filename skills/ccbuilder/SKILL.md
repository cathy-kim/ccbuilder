...
**상세**: [references/mcp-guide.md](references/mcp-guide.md) | **v2.1.119**: 서브에이전트·SDK MCP 서버 재구성 시 병렬 연결; HTTP/SSE/WebSocket 서버 `headers`의 `${ENV_VAR}` 플레이스홀더 치환 수정; OAuth `--client-secret` 토큰 교환 시 전송 수정
...
---
**Status**: 2026-03-27 | **Claude Code**: v2.1.119+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
