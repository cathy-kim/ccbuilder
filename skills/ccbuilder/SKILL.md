...
| **claude mcp serve** | Claude Code를 MCP 서버로 노출; **`alwaysLoad`**: 서버 모든 도구 tool-search 지연 없이 항상 로드 (v2.1.121); 시작 오류 시 최대 3회 자동 재시도 (v2.1.121); **`workspace`** 예약 서버 이름 (충돌 시 경고·생략, v2.1.128); `tools/list` 실패 시 1회 재시도 + "connected · tools fetch failed" 표시 (v2.1.132); 비인가 connectors "needs auth" 표시 (v2.1.132); stdio 서버 비프로토콜 stdout → 무제한 메모리 증가 수정 (v2.1.132) |
...
---
**Status**: 2026-05-03 | **Claude Code**: v2.1.132+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
