...
| **claude mcp serve** | Claude Code를 MCP 서버로 노출; **`alwaysLoad`**: 서버 모든 도구 tool-search 지연 없이 항상 로드 (v2.1.121); 시작 오류 시 최대 3회 자동 재시도 (v2.1.121); stdio 서버에 `CLAUDE_PROJECT_DIR` 자동 제공, plugin config `${CLAUDE_PROJECT_DIR}` 참조 가능 (v2.1.139); `workspace`는 예약된 서버 이름 — 기존 서버 경고 후 스킵 (v2.1.128); paginated `tools/list` 첫 페이지만 반환하여 도구 누락 수정 (v2.1.144); 비지원 MIME 타입(SVG) 이미지 → 디스크 저장 후 tool result 참조로 대화 복구 (v2.1.144); `.mcp.json` 파싱 실패 시 `claude mcp list` 구성 오류 표시 (v2.1.144); SDK/헤드리스 MCP pre-wait 병렬화 → 최대 2s 시작 개선 (v2.1.144) |
...
---
**Status**: 2026-05-03 | **Claude Code**: v2.1.144+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
