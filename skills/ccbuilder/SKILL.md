...
| **Hook → MCP 직접 호출** | `type: "mcp_tool"` 훅으로 MCP 도구 직접 실행 (v2.1.118); 서브에이전트·SDK 재구성 시 서버 병렬 연결 (v2.1.119); MCP 도구 호출 2분 초과 시 자동 백그라운드 전환 — `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`로 임계값·비활성화 설정 (v2.1.212); `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` — 첫 non-interactive 턴이 연결 중인 MCP 서버를 기다리는 시간 상한(`0`=대기 안 함, v2.1.274); 403 insufficient_scope 오류 시 만료 로그인 대신 누락 권한 명시 + `/mcp` 재인증 안내 (v2.1.274); 서버가 `listChanged` 미선언 상태로 list-changed 알림 전송 시에도 프롬프트·리소스 갱신 (v2.1.274); `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH` — 세션 전체 MCP 툴 설명·서버 지시문 2,048자 상한 변경 (v2.1.280); URL-mode elicitation — 2026-07-28 프로토콜에서 서버가 브라우저 기반 플로우 오픈 요청, 확인 불가 시 대기 다이얼로그 미표시 (v2.1.281); `claude plugin validate` — 드롭될 `.mcp.json` 엔트리·미선언 `${user_config.*}`·안전하지 않은 URL 검사 (v2.1.281); 도구 호출 백그라운드 전환 후 진행 알림 소실, 세션 종료 시 시작 중이던 stdio 서버 잔류, stateless 원격 서버 일시적 404로 세션 내내 사용불가, URL 없는 서버 사인인 시도 불명확 오류 버그 수정 + MCP 도구 반환 이미지 파일 저장(Bash·Read로 열람 가능) (v2.1.283) |
...
| **Project Memory** | CLAUDE.md + `@path` imports (5hop 재귀); **AGENTS.md 지원** — 프로젝트에 CLAUDE.md 없으면 AGENTS.md를 대신 읽음, `/config` → "Project instructions"에서 변경 (Bedrock·Vertex·Foundry 미지원, v2.1.277) |
...
| 기능 | 설명 |
|------|------|
| **Auto Memory** | `~/.claude/projects/<project>/memory/` 영구 저장; 서브디렉토리에서 시작한 세션의 auto-memory 노트 편집이 민감 파일 쓰기로 오차단되던 버그 수정 (v2.1.283) |
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.283+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
