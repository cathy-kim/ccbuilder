...
| **MCP OAuth** | 콜백 포트 충돌·재인증 수정 (v2.1.74); CIMD/SEP-991 (Client ID Metadata Document) 지원 (v2.1.81); RFC 9728 Protected Resource Metadata discovery — 인증 서버 자동 탐색 (v2.1.85); strict authorization 서버 대응 — redirect URI `localhost`→`127.0.0.1` (v2.1.229); 사전 등록된 OAuth 클라이언트(Slack 등) redirect URI mismatch 로그인 실패 수정 (v2.1.231) |
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.231+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
