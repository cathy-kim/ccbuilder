...
**Version**: 2.57.0
**Last Updated**: 2026-09-15 (v2.1.272)
...
| 기능 | 설명 |
|------|------|
| **HTTP Transport** | 권장 transport (`--transport http`), SSE deprecated; 레거시 HTTP+SSE만 지원하는 서버는 자동 SSE fallback (v2.1.265) |
...
| **MCP OAuth** | 콜백 포트 충돌·재인증 수정 (v2.1.74); CIMD/SEP-991 (Client ID Metadata Document) 지원 (v2.1.81); RFC 9728 Protected Resource Metadata discovery — 인증 서버 자동 탐색 (v2.1.85); 사인인 필요한 원격 서버에 실제 인증 전까지 OAuth 클라이언트 미등록 (v2.1.265); 로컬 콜백 포트 바인딩 불가 시 사인인 실패 수정, `/mcp`·`claude mcp list`·로그인 오류의 `${VAR}` 시크릿 노출 수정 (v2.1.268); 클라이언트 등록 처리(동의 거부·redirect URI 재사용·동시 쓰기) 버그 수정, MCP 도구 bare name 선택 시 tool search 매칭 실패 수정 (v2.1.271) |
...
| `TaskGet` | 개별 작업 상세 조회 — 위 4개 도구·`TodoWrite`는 Claude 3.x·Opus 4.0-4.7·Sonnet 4.0-4.6·Haiku 4.5에서만 기본 제공, 그 외 모델은 `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` 필요 (v2.1.268) |
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.272+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
