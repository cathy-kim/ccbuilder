...
멀티 에이전트 팀 협업 시스템. Team Lead가 Teammate들을 조율하여 병렬 작업 수행. **v2.1.72**: 팀 에이전트 리더 모델 자동 상속. **v2.1.114**: 팀메이트 도구 권한 요청 시 권한 다이얼로그 크래시 수정. **v2.1.232**: `/config` "Default teammate model" 설정 제거 — 팀메이트는 spawn에서 명시하지 않으면 리더 모델을 그대로 사용.
...
### Task Management (신규)
> **v2.1.233 Breaking Change**: Opus 4.8·Sonnet 5·Fable 5·Mythos 5 등 최신 모델에서 Task/Todo 도구가 **기본 비활성화**됨 — `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`로 복원.
| 도구 | 용도 |
|------|------|
...
---
**Status**: 2026-06-18 | **Claude Code**: v2.1.237+ | **Sync**: [version-sync.md](references/version-sync.md) | [check-updates.sh](../../scripts/check-updates.sh)
